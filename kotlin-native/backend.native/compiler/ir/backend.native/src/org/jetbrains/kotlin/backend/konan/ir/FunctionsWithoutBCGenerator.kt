...
internal class FunctionsWithoutBCGenerator(val context: KonanBackendContext) {
    ...
    private fun generateFunction(baseFunction: IrSimpleFunction) =
            context.irFactory.createFunction(
                    ...
                    KonanNameConventions.setWithoutBC,
                ...
                function.dispatchReceiverParameter = baseFunction.dispatchReceiverParameter
                ...
                val setWithoutBEAnnotations = baseFunction.annotations.map { annotation ->
                    annotation.deepCopyWithSymbols().also { copy ->
                        if (copy.isAnnotationWithEqualFqName(KonanFqNames.gcUnsafeCall)) {
                            // Transform `GCUnsafeCall` annotation to point to right native function.
                            val parameter = copy.symbol.owner.valueParameters.single { it.name.asString() == "callee" }
                            copy.symbol.owner.valueParameters = listOf(
                                    IrValueParameterImpl(
                                            copy.startOffset, copy.endOffset,
                                            parameter.origin,
    ...
    fun generate() {
        ...
        arraysClasses.forEach { classSymbol ->
            ...
            classSymbol.owner.addMember(generateFunction(setFunction))

            val getFunction = classSymbol.owner.functions.single { it.descriptor.name == OperatorNameConventions.GET }
            classSymbol.owner.addMember(generateFunction(getFunction))
        }
    }
}